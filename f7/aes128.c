...
// direct keygenassist, so we expand the schedule in scalar C. The resulting
// 11 round keys are byte-identical to the AES-NI schedule, so ciphertext
// matches bit-for-bit.

static const uint8_t f7_sbox[256] = {
...
static const uint8_t f7_rcon[11] = {0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36};

// Expand 16-byte key into 176 bytes (11 round keys).
static void key_expansion(const uint8_t *key, uint8_t rk[176]) {
    ...
    while (bytes < 176) {
        ...
        if (bytes % 16 == 0) {
            ...
            // SubWord
            for (int i = 0; i < 4; i++) t[i] = f7_sbox[t[i]];
            t[0] ^= f7_rcon[rconi++];
        }
        for (int i = 0; i < 4; i++) {
...
// vaeseq_u8(state, key) = (state XOR key) -> SubBytes -> ShiftRows
// vaesmcq_u8(state)     = MixColumns
// Round structure differs from x86: key is XORed at the START of vaese.
// So we feed round_keys[i] into vaese, and the AddRoundKey for round i
// happens inside the NEXT vaese (or as a final XOR for the last round).
...
void aes128(uint8_t *plaintext, uint8_t *key, uint8_t *ciphertext) {
    ...
    // Rounds 0..8: AddRoundKey(rk_i) + SubBytes + ShiftRows + MixColumns
    for (int i = 0; i < 9; i++) {
        block = vaeseq_u8(block, vld1q_u8(rk + 16 * i));
        block = vaesmcq_u8(block);
    }
    // Round 9: AddRoundKey(rk_9) + SubBytes + ShiftRows (NO MixColumns)
    block = vaeseq_u8(block, vld1q_u8(rk + 16 * 9));
    // Final AddRoundKey with rk_10
    block = veorq_u8(block, vld1q_u8(rk + 16 * 10));
