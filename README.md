...
**Base image:** [`al3monni/kria-base:22.04`](https://hub.docker.com/r/al3monni/kria-base) (arm64/v8)
...
| `e5edc89` | Dockerfile: `build-essential` added — `gcc` alone omitted `make` and `libc6-dev` | M-A12 |
| `aaf1af7` | Dockerfile: base image → `ubuntu:22.04` | M-A13 |
| `ad1385f` | Dockerfile: base image → custom Kria Ubuntu (`al3monni/kria-base:22.04`) | M-A14 (§11) |

The ARM work therefore falls into three phases: **make it build** (`9207e41` … `c992b51`), **make it register and run correctly** (`af69ca9` … `9a902ab`), and **move it onto the real board's userspace** (`e5edc89` … `ad1385f`).

---

## 11. Base image migration [hw]

The final phase of the port moved the container off a generic Ubuntu base and onto a snapshot of the board's own userspace. Three commits, each fixing what the previous one exposed.

### M-A13 — base image to `al3monni/kria-base:22.04` [hw] · `Dockerfile` · `ad1385f`
Final step: the base is now a snapshot of a running Kria board's root filesystem, published to Docker Hub.

```dockerfile
FROM al3monni/kria-base:22.04
```

**What the image is.** A rootfs tarball taken from a live Kria board running Ubuntu 22.04 IoT, brought into Docker with `docker import` and pushed to [Docker Hub](https://hub.docker.com/r/al3monni/kria-base). Platform `linux/arm64/v8`, digest `626d15cb9375`, 4.64 GB compressed.

**Why.** Stock `ubuntu:22.04` is the same distribution but not the same userspace: it lacks the Xilinx-specific tooling that ships with AMD's Kria image (`xmutil` and the platform-statistics utilities among them), which the power-measurement work in §9 will need. Building on a frozen snapshot also makes the toolchain reproducible across board reflashes — the container no longer depends on what happens to be installed on the board at build time.

**How it was produced.** Tar the live root filesystem with the volatile and self-referential paths excluded, then import. Two exclusions matter and are easy to miss: `/var/lib/docker` **and** `/var/lib/containerd` — omitting the second captures the live image store inside the snapshot and bloats it enormously — plus snap blobs. Verify the result before pushing, since `docker import` on an x86 host will happily mislabel an arm64 rootfs:

```bash
docker inspect al3monni/kria-base:22.04 --format '{{.Architecture}}'   # -> arm64
```

**How to use it.** Treat it as a frozen versioned base. All application build steps stay in the git-tracked `Dockerfile` layered on top; nothing gets baked into the snapshot, or the build stops being reproducible from source.

```bash
docker pull al3monni/kria-base:22.04
```

Budget disk space on the board's microSD accordingly — the pull alone is several GB before any application layer is built.
